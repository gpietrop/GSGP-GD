...
function experiment(dname, n, outfile, num_gen_1, num_gen_2, learning_rate, population_size, k1, k2)
    ...
    mutation_prob = 0.3
    mutation_step = 0.1
    num_gen = div((num_gen_1 + num_gen_2), k1 + k2)
    ...
    hist_GP = Array{Any,1}(nothing, num_gen_1 + num_gen_2 + 1)
    hist_HYB = Array{Any,1}(nothing, num_gen + 1)
    hist_NEW_1 = Array{Any,1}(nothing, num_gen_1 + 1)
    hist_NEW_2 = Array{Any,1}(nothing, num_gen_2 + 1)
    
    res_GP = Array{Any,1}(nothing, num_gen_1 + num_gen_2 + 1)
    res_HYB = []
    res_NEW = Array{Any,1}(nothing, num_gen_1 + num_gen_2 + 1)
    
    hist_GP[1] = P
    hist_HYB[1] = P
    hist_NEW_1[1] = P
    
    out_GP = open(outfile * "-GP", "w")
    out_HYB = open(outfile * "-HYB", "w")
    out_NEW = open(outfile * "-NEW", "w")
    out_TOT = open(outfile, "w")
    out_res = open(outfile*"-res", "w")
    
    print("lr = $(learning_rate)\n pop = $(population_size)\n GEN1 = $(num_gen_1)\n GEN2 = $(num_gen_2)\n")
    print(out_TOT, "lr = $(learning_rate)\n pop = $(population_size)\n GEN1 = $(num_gen_1)\n GEN2 = $(num_gen_2)\n") 
    print(out_res, "lr = $(learning_rate)\n pop = $(population_size)\n GEN1 = $(num_gen_1)\n GEN2 = $(num_gen_2)\n")        
    
    print("\nONLY GSGP\n")
    print(out_TOT, "\nONLY GSGP\n")
    print(out_res, "\nONLY GSGP\n")
    for i=2:(num_gen_1 + num_gen_2 + 1)
        hist_GP[i] = next_generation(hist_GP[i-1], conf, d.train_x, d.train_y)
        f_train, best_idx = best_fitness(hist_GP[i], d.train_x, d.train_y)
        res_GP[i-1] = f_train
        f_test = individual_fitness(hist_GP[i], d.test_x, d.test_y, best_idx)
        print("$(i-1)\t$(f_train)\t$(f_test)\n")
        print(out_TOT, "$(i-1)\t$(f_train)\t$(f_test)\n")
        print(out_GP, "$(i-1)\t$(f_train)\t$(f_test)\n")
        if i == (num_gen_1 + num_gen_2 + 1)
            print(out_res, "$(i-1)\t$(f_train)\t$(f_test)\n")
        end
    end
    ...
    for i=2:num_gen + 1
        for j=1:k1
            hist_HYB[i] = next_generation(hist_HYB[i-1], conf, d.train_x, d.train_y)
            ...
            print(out_TOT, "$(i-1)\t$(j)\t$(f_train)\t$(f_test)\n")
            print(out_HYB, "$(i-1)\t$(j)\t$(f_train)\t$(f_test)\n")
        end
        for j=k1+1:k1+k2
    ...
    print(out_res, "\nNEW CASE\n")
    for i=2:num_gen_1 + 1
        hist_NEW_1[i] = next_generation(hist_NEW_1[i-1], conf, d.train_x, d.train_y)
        
        f_train, best_idx = best_fitness(hist_NEW_1[i], d.train_x, d.train_y)
        f_test = individual_fitness(hist_NEW_1[i], d.test_x, d.test_y, best_idx)
        print("$(i-1)\t$(f_train)\t$(f_test)\n")
        print(out_TOT, "$(i-1)\t$(f_train)\t$(f_test)\n")
        print(out_NEW, "$(i-1)\t$(f_train)\t$(f_test)\n")
        if i == (num_gen_1 + 1)
            print(out_res, "$(i-1)\t$(f_train)\t$(f_test)\n")
        end
    end
    hist_NEW_2[1] = last(hist_NEW_1)
    for i=2:num_gen_2 + 1
