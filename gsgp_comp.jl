...
function experiment(dname, n, outfile, num_gen_1, num_gen_2, learning_rate, population_size, k1, k2)
    ...
    hist_gsgp = Array{Any,1}(nothing, num_gen_1 + num_gen_2 + 1)
    hist = Array{Any,1}(nothing, num_gen + 1)
    hist_1 = Array{Any,1}(nothing, num_gen_1 + 1)
    hist_2 = Array{Any,1}(nothing, num_gen_2 + 1)
    ...
    for i=2:(num_gen_1 + num_gen_2 + 1)
        hist_gsgp[i] = next_generation(hist_gsgp[i-1], conf, d.train_x, d.train_y)
        f_train, best_idx = best_fitness(hist_gsgp[i], d.train_x, d.train_y)
        res_GP[i-1] = f_train
        f_test = individual_fitness(hist_gsgp[i], d.test_x, d.test_y, best_idx)
        print("$(i-1)\t$(f_train)\t$(f_test)\n")
        print(out, "$(i-1)\t$(f_train)\t$(f_test)\n")
    end
    ...
    print(out_res, "\nNEW CASE\n")
    for i=2:num_gen_1 + 1
        hist_1[i] = next_generation(hist_1[i-1], conf, d.train_x, d.train_y)
        
        f_train, best_idx = best_fitness(hist_1[i], d.train_x, d.train_y)
        f_test = individual_fitness(hist_1[i], d.test_x, d.test_y, best_idx)
        print("$(i-1)\t$(f_train)\t$(f_test)\n")
        print(out, "$(i-1)\t$(f_train)\t$(f_test)\n")
    end
    hist_2[1] = last(hist_1)
    for i=2:num_gen_2 + 1
        par = extract_params(hist_2[i-1])
        f = make_loss(hist_2[i-1], d.train_x, d.train_y)
        g = gradient(f, par)[1]
        hist_2[i] = update_params(hist_2[i-1], g, learning_rate)
        
        f_train, best_idx = best_fitness(hist_2[i], d.train_x, d.train_y)
        f_test = individual_fitness(hist_2[i], d.test_x, d.test_y, best_idx)
        print("$(i-1)\t$(f_train)\t$(f_test)\n")
        print(out, "$(i-1)\t$(f_train)\t$(f_test)\n")
    end
    # print(out_res, "$(f_train)\t$(f_test)\n")
    ...
    # plot(res_GP, label="GP")
    # plot!(res_HYB, label="HYBRID")
